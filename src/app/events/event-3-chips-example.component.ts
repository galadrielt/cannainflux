...
export class ChipListValidationExample implements OnInit {
 ...
 data = {
  ...
  ngOnInit() {
      ...
      for (this.k in <any>this.data.names[1]) {
        this.divsAvail.push(this.data.names[1][this.k].name);
      }

      this.myForm = this.fb.group({
      //Commented out because creates unnec.
      //names: this.fb.array(this.data.names, this.validateArrayNotEmpty)
      names: this.fb.array([this.initName('Divisions',this.divsAvail)])
    });
    ...
    if (this.data.names[1]){
       for (this.i in <any>this.data.names[1]) {
         this.addNewChipList(<any>this.data.names[1][this.i].name);
         //this.initName('High School',['A', 'B']);
          ...
          for (this.j in this.data.names[1][this.i].levels) {
            ...
            console.log(this.data.names[1][this.i].levels[this.j].name);
              ...
              for (this.z in this.data.names[1][this.i].levels[this.j].breakdowns){
                ...
                this.addChip(this.data.names[1][this.i].levels[this.j].breakdowns[this.z], <FormControl>this.myForm.controls[this.data.names[1][this.i].levels[this.j].name]);
                bkdowns.push(this.data.names[1][this.i].levels[this.j].breakdowns[this.z]);
              }
  ...
  initChip(arr: [], ctrl: FormControl): void {

    for(let ar of arr){
    // Add name
      if ((ar || '').trim()) {
        console.log("AR:", ar, ctrl);
        const control = ctrl;
        control.value.val.push(ar);
